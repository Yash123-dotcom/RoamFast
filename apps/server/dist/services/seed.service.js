...
export class SeedService {
                ...
                if (!exists) {
                    await prisma.hotel.create({
                        data: {
                            ...
                            ownerId: demoUser.id,
                            status: 'APPROVED', // Approve by default for demo
                            rooms: {
                                create: [
    ...
    async seedCommissionData() {
        ...
        try {
            ...
            // Get or create a few test users
            const testUsers = await Promise.all([
                prisma.user.upsert({
                    where: { email: 'guest1@example.com' },
                    update: {},
                    create: { email: 'guest1@example.com', name: 'Sarah Johnson', role: 'USER' }
                }),
                prisma.user.upsert({
                    where: { email: 'guest2@example.com' },
                    update: {},
                    create: { email: 'guest2@example.com', name: 'Michael Chen', role: 'USER' }
                }),
                prisma.user.upsert({
                    where: { email: 'guest3@example.com' },
                    update: {},
                    create: { email: 'guest3@example.com', name: 'Emily Rodriguez', role: 'USER' }
                })
            ]);
            // Get approved hotels
            ...
            if (hotels.length === 0) {
                throw new Error('No approved hotels found. Please seed hotels first.');
            }
            let bookingsCreated = 0;
            let commissionsCreated = 0;
            // Create sample bookings with commissions
            const bookingData = [
                { userId: testUsers[0].id, hotelId: hotels[0].id, price: 25000, status: 'PAID', daysAgo: 5 },
                { userId: testUsers[1].id, hotelId: hotels[1]?.id || hotels[0].id, price: 45000, status: 'PAID', daysAgo: 3 },
                { userId: testUsers[2].id, hotelId: hotels[2]?.id || hotels[0].id, price: 38000, status: 'PENDING', daysAgo: 1 },
                { userId: testUsers[0].id, hotelId: hotels[3]?.id || hotels[0].id, price: 85000, status: 'PAID', daysAgo: 10 },
                { userId: testUsers[1].id, hotelId: hotels[4]?.id || hotels[0].id, price: 95000, status: 'PENDING', daysAgo: 2 },
                { userId: testUsers[2].id, hotelId: hotels[0].id, price: 32000, status: 'FAILED', daysAgo: 7 },
            ];
            for (const data of bookingData) {
                ...
                if (!existingBooking) {
                    ...
                    const platformFee = 500; // Flat fee
